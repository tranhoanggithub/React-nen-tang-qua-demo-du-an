...
export default class BaiTapRenderFilms extends Component {
  renderFilms = () => {
    let contentFims = DataFilms.map((film, index) => {
      return (
        <div className="col-3" key={index}>
          <div className="card text-white bg-primary" style={{width:'200px'}}>
            <img style={{with:'150px'}} className="card-img-top" src={film.hinhAnh} alt={film.tenPhim} />
            <div className="card-body">
              <h4 className="card-title" style={{fontSize:'17px'}}>{film.tenPhim}</h4>
              <p className="card-text" style={{fontSize:'13px'}}>{film.moTa}</p>
            </div>
          </div>
        </div>
      );
      ...
      >
        <div style={{ backgroundColor: "rgba(0,0,0,.5)", minHeight: "2000px" }}>
          <nav
            ...
            style={{ backgroundColor: "rgb(87,83,149,0.8)" }}
          >
